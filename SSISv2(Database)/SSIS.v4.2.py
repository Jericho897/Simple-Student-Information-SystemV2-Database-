...
class MainWindow(QMainWindow):
    ...
    def delete_course(self):
        ...
        # Get the IDs of students enrolled in the course being deleted
        ...
        # Set the course code of enrolled students to "N/A"
        connection = sqlite3.connect(self.student_database)
        cursor = connection.cursor()
    ...
    def add_student(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if not all([student_id, student_name, gender, year, course_code]):
            ...
            cursor.execute("INSERT INTO students VALUES (?, ?, ?, ?, ?)",
                           (student_id, student_name, gender, year, course_code))
            connection.commit()
            connection.close()
    ...
    def update_student(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            new_student_name = student_name_input.text().strip()
            new_gender = gender_input.currentText()
            new_year = year_input.currentText()
            new_course_code = course_code_input.currentText()

            if not all([new_student_name, new_gender, new_year, new_course_code]):
                QMessageBox.warning(self, "Error", "All fields are required!")
                return

            connection = sqlite3.connect(self.student_database)
            cursor = connection.cursor()
            cursor.execute("UPDATE students SET StudentName=?, Gender=?, Year=?, CourseCode=? WHERE StudentID=?",
                           (new_student_name, new_gender, new_year, new_course_code, student_id))
            connection.commit()
            connection.close()
